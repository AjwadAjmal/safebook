...
## Akzeptanzkriterien
- [x] Middleware/Session-Check leitet Nutzer ohne Haushalt zu `/onboarding`.
- [x] Onboarding-Seite mit Option "Haushalt erstellen".
- [x] Server Action erstellt neuen Eintrag in `households`.
- [x] Aktueller Nutzer wird in der DB mit der neuen `household_id` verknüpft und bekommt die Rolle `admin`.
- [x] Nach Erstellung erfolgt die Weiterleitung zum Haupt-Dashboard.
